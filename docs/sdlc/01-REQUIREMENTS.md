...
**FR-27 — Effective severity is worse-of, but ONLY for genuinely at-risk postures.** A finding's severity MUST be `SeverityFromPosture(posture)`, with the Mosca/HNDL bump (`HighestSeverity` against `SeverityFromMosca`) applied ONLY when the posture is NOT already quantum-resistant. Quantum-resistant postures (`symmetric-only`/`pqc-hybrid`/`pqc-ready`) keep their posture severity (INFORMATIONAL) regardless of data shelf-life, because the cryptography is already quantum-resistant; the genuinely vulnerable postures (`no-encryption`/`legacy-tls`/`non-pqc-classical`/`unknown`) keep the full worse-of behavior. *Impl:* `internal/risk/severity.go:52-57` (HighestSeverity), `:42-49` (IsQuantumResistantPosture); gated at `internal/scanner/findings.go:47-50`. **Note:** previously the worse-of was applied *unconditionally*, so a long-lived `symmetric-only` store (e.g. RDS Mosca = 9) was wrongly stamped CRITICAL — a quantum-resistant AES-256 store flagged as a top vulnerability. That same store is now correctly INFORMATIONAL (verified on a mock scan: quantum-resistant-stamped CRITICAL/HIGH 38→0, total assets unchanged, so the asset is still inventoried — just no longer over-alarmed).
...
**FR-29 — Single pure finding generator.** `BuildFindings([]CryptoAsset, *compliance.Registry, overrides) → []Finding` MUST be the only finding source; it MUST be dependency-light (stdlib + internal/risk + internal/compliance + pkg/models) and produce deterministic *classification* (posture, worse-of severity, Mosca score, compliance mappings) for identical input. *Impl:* `internal/scanner/findings.go:28-97`. **Deterministic ID:** `Finding.ID = stableFindingID(a, posture)` (`findings.go:76`, helper `findings.go:99-121`) is a run-independent content key — `finding:<key>:<posture>`, where `<key>` is the asset's `BomRef` (or an FNV-64a hash of account|region|service|resourceID when absent) — so finding artifacts diff cleanly across runs. **Caveat (not byte-identical):** the only volatile fields are the per-run timestamps `CreatedAt`/`UpdatedAt = time.Now().UTC()` (one `now` per call, `findings.go:29,92-93`). So the deterministic/identical guarantee covers the *classification content* and the stable `ID`, NOT the timestamps; purity/identity tests MAY rely on the stable `ID` and MUST exclude only the timestamps.

**FR-30 — Finding links back to its asset.** Each `Finding` MUST carry `AssetBomRef` linking to `CryptoAsset.BomRef`, plus account/region/service/resource identity, Mosca, compliance mappings, recommendation, and docs URL. *Impl:* `pkg/models/finding.go:66-85`; `internal/scanner/findings.go:75-94`.

**FR-31 — Identical classification across live / mock / offline.** The same `BuildFindings` MUST be reused by the engine, mock engine, and offline org-merge-files so the *classification* (posture, severity, Mosca, compliance) is identical regardless of path. The findings are NOT byte-identical: per FR-29, the `ID` is a deterministic content key but each call stamps current timestamps. *Impl:* live `internal/scanner/engine.go:235-237`; mock `internal/scanner/mock_engine.go:34`; offline `cmd/cryptamap/org_merge_files.go:97`. *Test:* `cmd/cryptamap/purity_test.go`.
...
**FR-42 — Asset-aware quick-win KPI.** "Quick win" MUST require both a one-flip service AND an effective (asset-aware) status that is genuinely actionable; a quantum-resistant asset on a one-flip service MUST NOT inflate the KPI. *Impl:* `internal/roadmap/roadmap.go:288-293`, `EffectivePQCStatus` use `:118`.
...
**FR-45b — PQ evidence tier (capable vs confirmed).** A PQ-hybrid posture derived from a security-policy NAME or AWS-doc capability means the endpoint *permits* post-quantum key exchange, not that a PQ handshake was *observed*. The CBOM MUST stamp `cryptamap:pqEvidence` = `confirmed` (observed negotiation — only `cloudtrail_evidence`) vs `capable` (config/policy permits) on PQ-hybrid assets, so the headline "% quantum-resistant" never reports a permitted-but-unproven endpoint as proven. (Active probing + CloudTrail mining to grow the `confirmed` set is a v2 feature.) *Impl:* `internal/output/cyclonedx.go` `pqEvidenceProps`; dashboard badge in `AssetDetailPanel.tsx`. *Test:* `internal/output/crypto_graph_test.go` (`TestPQEvidence_CapableVsConfirmed`).
...
**FR-67 — Default scan policy.** Defaults MUST include `MaxGoroutines=50`, rate limiting {MaxRetries:5, BaseDelayMs:100, MaxDelayMs:30000} (jitter is always applied by the engine and is not configurable), a Security Hub `product_arn` stamped into local ASFF findings (no live BatchImportFindings push), formats {CycloneDX, PQCCExcel, ASFF, Roadmap, HTML = true; PDF = false}, LocalDir `./dist/scan-output`, Mosca 7/2/3, and 9 frameworks. The S3/DynamoDB evidence-store writers (Lambda fan-out path) are configured via the `RESULTS_BUCKET`/`SCANS_TABLE`/`RETENTION_DAYS` env vars set by the CDK, not via YAML. *Impl:* `internal/config/loader.go`.
...
**NFR-1 — Determinism & purity of core transforms.** Finding generation, merge, and roadmap MUST be pure (stdlib + internal/* only), free of I/O and AWS SDK imports, and produce identical *classification/ordering* for identical input. (As noted in FR-29, `BuildFindings` emits a deterministic `ID` but current timestamps per call, so serialized findings are not byte-identical — the determinism guarantee is over the classified content, the stable `ID`, and sort order, not the raw bytes.) *Impl:* `internal/scanner/findings.go` header; `internal/merge/merge.go:1-18`; `internal/roadmap/roadmap.go:1-25`. *Test:* `cmd/cryptamap/purity_test.go`, `internal/pqc/purity_test.go`, `internal/merge/streaming_test.go`.
...
| FR-27 | Worse-of severity, quantum-resistant-gated | `internal/risk/severity.go:42-49,52-57`; `findings.go:47-50` | `internal/scanner/findings_test.go`, `internal/risk/severity_test.go` |
...
3. **`MoscaOverrides` is now wired from config in the CLI (previously a gap).** CLI YAML `risk.mosca.overrides` flows through `Config.MoscaOverrideParams()` (`internal/config/mosca.go`) into `EngineOptions.MoscaOverrides` (`cmd/cryptamap/main.go:158`) and then into `BuildFindings`; the mapping and behavior are tested in `internal/config/mosca_test.go`. The Lambda handler wires the same option (`cmd/cryptamap/lambda.go:195`) but currently uses default config, so custom Mosca overrides remain a CLI/config-file feature unless Lambda config loading is later added.
