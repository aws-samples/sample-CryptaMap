...
8. **Build findings (shared pure path).** `buildFindings → BuildFindings` reads `posture` from `Properties` (default `unknown→MEDIUM`), computes Mosca `X+Y-Z`, and sets severity from the posture — applying the **worse-of** Mosca/HNDL bump **only for non-quantum-resistant postures** (the AES-256/PQC quantum-resistant postures stay informational; see §7 and Journey C step 5), then attaches compliance mappings (`engine.go:235-237`; `findings.go:29-77`, `:39-50`; `risk/mosca.go:12-32`; `risk/severity.go:42-57`).
...
sequenceDiagram
    ...
    loop each region
        ...
        BF-->>Eng2: []Finding (severity: posture, worse-of Mosca only if not quantum-resistant)
...
6. **Hierarchical merge (tier 1).** `AccountMergeFanout` invokes the *same* scanner Lambda with `mergeAccount:true` per distinct account; each child streams that account's `scans/raw/<runId>/<accountId>-*` shards into one per-account merged object under `scans/account-merged/<runId>/` (`lambda.go:85-89`; `lambda_event.go:69-73,131-148`; `org-fanout-stack.ts:377-433`). One account per child = no single-merge OOM cliff ([`../SCALING.md`](../SCALING.md) §4.1).
7. **Final merge (tier 2).** `BuildOrgCbom` invokes the scanner with `merge:true` + `expectedShards`; it streams the per-account merged objects (falling back to raw shards), reconciles the **completion barrier** (observed vs expected shards), and emits the org CBOM + roadmap + coverage under `scans/latest/<runId>.*` (`lambda.go:90-92`; `lambda_event.go:59-79`; `org-fanout-stack.ts:435-467`). The Go merge is the sole producer of `scans/latest/<runId>.*` — a prior Node `MergeResults` counts-rollup Lambda was removed as redundant and over-privileged (`org-fanout-stack.ts:304`).

> **Same merge math, two entry points.** Both the Lambda merge and the offline `org-merge-files` (Journey C) reuse the same pure `merge.Merge`, but they source findings differently: `org-merge-files` consumes CBOMs (lossy for findings) and regenerates findings via `scanner.BuildFindings` (`org_merge_files.go:112`), while the Lambda merge consumes RAW `ScanResult` JSON and merges the shard findings verbatim without re-deriving them (`lambda_merge_core.go:171-178`). Classification is **identical** (posture / severity / Mosca / compliance) either way, because the raw shards' findings were built by the same pure `BuildFindings` at scan time. The *records* are not byte-identical across regenerations: `BuildFindings` stamps `CreatedAt/UpdatedAt: time.Now().UTC()` (one `now` per call, `findings.go:29,92-93`), so those timestamps differ per run. `Finding.ID` is now a **deterministic** content key (`ID: stableFindingID(a, posture)`, `findings.go:76`, helper `findings.go:99-121`), so both the input-derived classification fields and the `ID` are reproducible — only the timestamps differ per run.
...
2. **Why the merge regenerates findings.** The CBOM carries assets, not findings, so findings *must* be re-derived to populate the roadmap — and because `BuildFindings` is pure and dependency-light, the offline merge produces **identical classification** (posture / severity / Mosca / compliance) to a live scan from the same assets (`org_merge_files.go:88-99`; `findings.go:28-97`). The findings are *not* byte-identical: each call sets `CreatedAt/UpdatedAt: time.Now().UTC()` (one `now` per call, `findings.go:29,92-93`). `Finding.ID` is now a **deterministic** content key (`ID: stableFindingID(a, posture)`, `findings.go:76`, helper `findings.go:99-121`), so purity tests may compare classification content plus the stable `ID` and need only exclude the two timestamps.
...
flowchart TD
    ...
    SV --> R2["Severity = posture; worse-of Mosca only if not quantum-resistant"]
...
6. **Build & test.** `go build ./...` (the plain build must stay green — lambda code is build-tagged), `make test` runs `go test ./internal/... ./pkg/... ./cmd/...` (`Makefile:86-87`, scoped by `GOPACKAGES` at `Makefile:3`). `make check-types` guards the Go→TS model sync, and `make check-knowledge` guards embedded PQC-knowledge staleness (`Makefile:37-45`).
...
- **Severity = posture severity, with a worse-of Mosca/HNDL bump applied only to genuinely-vulnerable postures.** The bump is skipped for quantum-resistant postures (`symmetric-only` / `pqc-hybrid` / `pqc-ready`) so AES-256 stores are not false-CRITICAL (`findings.go:39-50`; `risk.IsQuantumResistantPosture` at `risk/severity.go:42-49`; corrected — was previously an unconditional worse-of).
