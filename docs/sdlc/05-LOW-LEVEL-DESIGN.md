...
- Derives a representative `primitive` from the underlying asset (`primitiveFor`, `internal/roadmap/roadmap.go:300-324`) and computes the asset-aware `EffectivePQCStatus` (`internal/pqc/lookup.go:30-64`) so a quantum-safe asset never carries the alarming "not-yet" badge.
...
3. **Severity is posture-first, with a *gated* Mosca bump** (`internal/scanner/findings.go:47-50`). The worse-of-two `HighestSeverity(posture, Mosca)` escalation fires **only for non-quantum-safe postures** (`!IsQuantumSafePosture`); a quantum-safe service (`symmetric-only`/`pqc-hybrid`/`pqc-ready`) keeps its posture severity (INFORMATIONAL) regardless of Mosca. **Note:** previously the worse-of was *unconditional*, so any high-Mosca service was CRITICAL even with benign quantum-safe posture — over-alarming inventoried AES-256 stores. Don't reintroduce the unconditional `HighestSeverity` here.
