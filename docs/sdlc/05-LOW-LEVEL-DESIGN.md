...
| `internal/services` | Shared scanner helpers: asset/property builders, provenance stampers, `MapConcurrent`, the truncation cap (`TruncationCapReached` + the `CapAndWarn` one-shot slice cap, backed by the machine-readable `TruncationSentinel`/`markTruncated`/`TakeTruncated` mark). | `internal/services/common.go` |
...
Independent of the optional `PerServiceCap`, every scanner is expected to self-guard with `services.TruncationCapReached(count, name, region)` (`internal/services/common.go:93`), which fires at `MaxAssetsPerScanner = 25000` (`internal/services/common.go:23`) with a **loud** stderr warning — because silent truncation = under-reported assets = a false "all clear". (Several older `keymgmt`/`sdkpqc`/`certmgmt` scanners still use a local `maxItems=1000` constant instead of the shared 25000 cap — see [Gotchas](#15-gotchas--invariants-you-must-not-break).)

Two supporting facilities harden this cap into a machine-readable signal rather than a bare stderr line:

- **`CapAndWarn[T](items, cap, scanner, region)` (`internal/services/common.go:115`)** is the one-shot helper for scanners that collect a full slice before capping. It closes the silent-truncation edge a bare `items[:cap]` leaves open: when `len(items) > cap` it routes through `TruncationCapReached(len(items), …)` so the loud warning + mark fire, *then* slices to `cap`; `cap<=0` is treated as `MaxAssetsPerScanner`. Adopt it anywhere a callsite currently does `return items[:cap], nil`.
- **A process-global truncation mark** makes truncation observable in the output. `TruncationCapReached` calls `markTruncated(scanner)` (`internal/services/common.go:44,98`); the engine consumes the mark per scanner via `TakeTruncated(scanner)` (`internal/services/common.go:68`) and appends the machine-readable `TruncationSentinel = "cryptamap:truncated"` prefix (`internal/services/common.go:31`) to that scanner's `ServiceScanReport`, so structured consumers can detect an under-reported shard without scraping stderr. `UnmarkTruncated` (`internal/services/common.go:58`) clears a stale mark when an attempt hit the cap and *then* errored before the engine consumed it (so a retry that succeeds under the cap is not falsely flagged).
...
`BuildFindings(assets, comp, overrides)` (`internal/scanner/findings.go:28-97`) is the *only* place assets become findings, and it is intentionally pure and dependency-light (stdlib `fmt`/`hash/fnv`/`time` + `internal/risk` + `internal/compliance` + `pkg/models`; it no longer imports `google/uuid`). For each asset:
...
5. Build the `Finding` (`internal/scanner/findings.go:75-94`) with a **deterministic** `ID = stableFindingID(a, posture)` (`findings.go:76`, helper `findings.go:99-121`), `Sprintf`-formatted title/description (`findings.go:77-78`), `Recommendation`/`DocsURL` (`findings.go:90-91`), and `CreatedAt`/`UpdatedAt` timestamps (`findings.go:92-93`). `stableFindingID` returns `finding:<key>:<posture>`, where `<key>` is the asset's `BomRef` (or an FNV-64a hash of `account|region|service|resourceID` when the ref is absent), so the id is run-independent and finding artifacts diff cleanly across scans — it is **no longer** the old per-call `uuid.NewString()`. The `Recommendation`/`DocsURL` strings come from the package-level helper funcs `recommendation(posture, service)` (`internal/scanner/engine.go:424-440`) and `docsURL(service)` (`internal/scanner/engine.go:474-479`) — these are plain functions (no `Engine` receiver) that `BuildFindings` calls directly; nothing in that helper region constructs a `Finding`. As part of the severity fix, `recommendation()` routes `PostureSymmetricOnly` through a dedicated `postureRecommendation` case (`engine.go:462-463`) that states AES-256/already-PQC at rest is quantum-resistant and needs no migration, so the INFORMATIONAL inventory findings carry an honest "listed for inventory completeness" recommendation rather than a migration nag.

Because it is pure, the **identical** function is reused by the live engine (via the `buildFindings` wrapper, `internal/scanner/engine.go:380-381`, called from `Run` at `engine.go:203`), the mock engine (`internal/scanner/mock_engine.go:34`), and the offline `org-merge-files` adapter (`cmd/cryptamap/org_merge_files.go:112`). That is the guarantee that lets the offline merge regenerate findings with **identical classification** (posture, severity, Mosca score, compliance mappings) from CBOM-derived assets (a CBOM carries assets, not findings). It is *not* byte-identical at the record level, but as noted in §1 the only volatile fields are the timestamps: `BuildFindings` stamps one per-call `CreatedAt`/`UpdatedAt = time.Now().UTC()` (`findings.go:29,92-93`), while `Finding.ID` is now the **deterministic** `stableFindingID(a, posture)` content key (`findings.go:76`), so it is stable run-to-run. (The wrapper's own code comment at `internal/scanner/engine.go:376-379` says the shared path gives "byte-identical behavior" — now that the id is deterministic that is accurate for the id and the classification, but the serialized record still differs on the two timestamps, so read "byte-identical" as *the shared code path*, not literal byte equality of the emitted JSON.)
...
Scanners attach provenance keyed into this knowledge via `StampDocFactKeyed(asset, key)` (`internal/services/common.go:285`), which writes `source=aws-doc` + the fact key (`PropDocFact`) and resolves confidence/url/asOf from the loaded knowledge by key — failing safe (key stamped, no fabricated date/confidence) when the key is unknown (`internal/services/common.go:291-304`). `StampObserved(asset, confidence)` (`internal/services/common.go:331`) marks a live-API-derived classification as the strongest basis (`source=observed`).

**Provenance is now also recorded on the protocol block, not only on the asset (C6/C7).** Two contract changes closed the earlier gaps where a doc-derived TLS verdict was self-unaware and a live LDAPS read was mislabeled as documentation:

- **`TLSProtocolPropsDoc(version, suiteName, confidence, docURL)` (`internal/services/common.go:454`) now stamps its own provenance onto the protocol block** — it sets `Source = "aws-doc"`, `DocConfidence`, and `DocSourceURL` on `ProtocolProperties` (`internal/services/common.go:456-458`) via the new `DocConfidence`/`DocSourceURL` fields (`pkg/models/asset.go:121-122`). Previously the confidence + doc URL were dropped from the block (the K-8 defect); now the doc-derived verdict is self-describing on the block itself even if a caller forgets the asset-level `StampDocFact`. These fields — like `Source` — are non-schema provenance stripped from the emitted CycloneDX node by `sanitizeForCDX` (protocolProperties is `additionalProperties:false`).
- **`directoryservice` now uses `StampObserved` for live LDAPS reads** instead of leaving an `aws-doc` source on the block. It seeds each directory with `TLSProtocolPropsDoc("", "directory-ldaps", "low", …)` for the Unknown/NotApplicable/SimpleAD fallbacks, but on a real per-region `DescribeLDAPSSettings` read (Enabled/Disabled) it **replaces the doc-fact seed props with the plain observed block** `TLSProtocolProps("", "directory-ldaps")` (`internal/services/transit/directoryservice.go:115`, and `NoEncryption()` for Disabled at :122) and stamps the asset via `services.StampObserved(&a, "high")` (`internal/services/transit/directoryservice.go:138-139`), so an observed verdict is never laundered as a doc-fact. Only the genuine fallbacks still `StampDocFact` at `low` confidence (`directoryservice.go:141-142`).
...
(`internal/services/common.go:93`.) For a fully-collected slice, use the one-shot `services.CapAndWarn(items, services.MaxAssetsPerScanner, s.Name(), region)` (`internal/services/common.go:115`) instead of a bare `items[:cap]`. For per-resource Describe fan-out, wrap the inner calls in `services.MapConcurrent` (§7) instead of a serial loop.
...
5. **`PerServiceCap` is not wired from config in the CLI path.** It is enforced in `Engine.Run` (`internal/scanner/engine.go:96-100`) but the CLI builds `EngineOptions` without populating it. (The previously-dead `config.Concurrency.PerServiceLimits` knob — never mapped to it — was removed on 2026-06-16.) `MoscaOverrides`, by contrast, **is** wired: CLI YAML `risk.mosca.overrides` flows through `Config.MoscaOverrideParams()` (`internal/config/mosca.go`) into `EngineOptions.MoscaOverrides` (`cmd/cryptamap/main.go:158`) and on to `BuildFindings` (`internal/scanner/engine.go:381`), with the mapping and end-to-end severity effect pinned by `internal/config/mosca_test.go`. The Lambda handler wires the same option (`cmd/cryptamap/lambda.go:195`) but currently runs on the default config, so custom Mosca overrides remain a CLI/config-file feature unless Lambda config loading is added later.
