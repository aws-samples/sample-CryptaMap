...
> **Reviewer takeaway:** CryptaMap's customer-resource scan surface is read-only by construction — no IAM action in CryptaMap's grant set can modify or delete scanned customer resources, and no AWS API call in the scan code mutates anything it scans. The only mutating grants are append-only evidence writes (`s3:PutObject`, `s3:Abort*`, `dynamodb:PutItem`) to the customer-owned evidence bucket and scans table (`cdk/lib/scanner-stack.ts:91-111`) — deliberately scoped with no delete/update/legal-hold/retention capability, so evidence can be appended but never tampered with. The strongest scan privilege it holds is *read-everything*, justified by crypto-detail completeness and bounded to read-only by the managed policy.
...
- `DataStack` is an **evidence store only** and "deliberately exposes NO query API" — it has no API Gateway, no query Lambda, no routes, no authorizer, and no presigned-URL path (`cdk/lib/data-stack.ts:12-24`). The crypto inventory is never reachable over the network from the deployed stack.
...
- CMK with `enableKeyRotation` and `RETAIN` removal — `cdk/lib/data-stack.ts:35-39`.
- Results bucket: `KMS` CMK encryption (`encryptionKey: this.dataKey`, `bucketKeyEnabled`), `BLOCK_ALL`, `versioned`, `RETAIN`, `enforceSSL`, server-access logs, retention lifecycle — `cdk/lib/data-stack.ts:65-83`.
- Scans table: customer-managed KMS, `RETAIN`, `deletionProtection`, PITR enabled — `cdk/lib/data-stack.ts:118-132`.
- Scanner Lambda log retention is six months (`cdk/lib/scanner-stack.ts:45-48`).

> **Note (writers now inherit the CMK; bucket policy denies non-KMS puts):** previously the Go S3 writers passed an explicit `ServerSideEncryption: AES256` on every `PutObject`, which **overrode the bucket's CMK default at put time** — so evidence actually landed under SSE-S3 (an AWS-owned key), not the customer-managed CMK this section claims. That override has been removed on **both** write paths: `internal/output/s3_writer.go:39-46` (per-scan CBOM) and `cmd/cryptamap/lambda_merge.go:259-266` (org merge) now send **no** `ServerSideEncryption`, so uploads inherit the bucket's default CMK encryption. As defense-in-depth, `DataStack` now also attaches a bucket resource policy that **DENIES any `s3:PutObject` whose `s3:x-amz-server-side-encryption` is not `aws:kms`** (Sid `DenyPutObjectWithoutKmsSse`, `iam.Effect.DENY` on `AnyPrincipal`) — `cdk/lib/data-stack.ts:93-104`, rationale at `:85-92`. Net effect: writers no longer downgrade at-rest encryption, and a future regression back to SSE-S3 (AES256) fails as a hard 403 rather than silently landing evidence under a weaker key. (A put with no SSE header at all still inherits the bucket default CMK — `StringNotEquals` does not match a missing key — so the Deny targets only the explicit wrong-algorithm case.)
...
1. **No query API exists** — `DataStack` is an evidence store only and "deliberately exposes NO query API" (`cdk/lib/data-stack.ts:12-24`): no API Gateway, no query Lambda, no routes, no presigned-URL path. There is nothing to authenticate against because there is nothing exposed.
...
3. **Evidence stays in a private, encrypted bucket** — collected CBOM/scan output lands in the `DataStack` results bucket with `BLOCK_ALL` public access, KMS-CMK encryption, and `enforceSSL` (`cdk/lib/data-stack.ts:65-83`), backstopped by a bucket policy that denies any non-`aws:kms` `PutObject` (`:93-104`, §4.2); it is read back only by an operator with explicit S3/KMS credentials.
...
CryptaMap is designed so that **customer cryptographic-inventory data never has to leave the customer's account or region.** To meet India data-residency obligations, the operator must deploy the evidence/home stack in an India region (`ap-south-1` or `ap-south-2`): the CDK emits a loud synth-time warning when the home region or any fan-out region falls outside India, but it does **not** hard-block deployment (a soft nudge, `cdk/bin/app.ts:99-120`). The mechanisms below keep data in-account/in-region.

### 6.1 Default model: data stays in-account / in-region

- **CLI / artifact-first:** the default `cryptamap` run is a local scan that writes CBOM / PQCC Excel / HTML / ASFF / roadmap / coverage artifacts to a local output directory; nothing is uploaded anywhere by the CLI itself. (Default output dir and the local-write path are in `cmd/cryptamap/main.go`, `writeArtifacts`.) On shared hosts these artifacts carry sensitive crypto-inventory detail, so the output directory is created `0700` and every artifact file is written `0600` (owner-only) — never world-readable — in both the CLI scan path and the offline `org-merge-files` path.
...
- **Region posture for India:** the data home is the `DataStack` region, which is `CDK_DEFAULT_REGION` — i.e. **operator-chosen**, not hardcoded to `ap-south-1`; an operator targeting India deploys the stacks in `ap-south-1` (or `ap-south-2`). The org fan-out defaults its scan-region list to `us-east-1,ap-south-1` (Mumbai), with `us-east-1` included only for genuinely global services (IAM, CloudFront) that surface there — `cdk/bin/app.ts:88-96` (fanout default + rationale comment). A synth-time data-residency nudge warns (but does not block) when the home or any fan-out region is outside India — `cdk/bin/app.ts:99-120`. The base config keeps the central/home region for writes and never repoints it to a scan region (`cmd/cryptamap/lambda.go:97-103`), so evidence is written in the home region.
...
- **Defense-in-depth response headers (every allowed response):** the loopback Host-guard also stamps a baseline header set on each response (`cmd/cryptamap/serve.go:253-268`): `Cache-Control: no-store` (the crypto inventory must never persist in a browser/proxy cache on a shared workstation), `X-Content-Type-Options: nosniff` (no MIME-sniffing into an executable type), `X-Frame-Options: DENY` + a `Content-Security-Policy` whose `frame-ancestors 'none'` is the load-bearing anti-clickjacking control (X-Frame-Options is the legacy backstop), `Referrer-Policy: no-referrer`, and a CSP that confines the SPA to its own origin (`default-src 'self'`; `connect-src 'self'` for the local-data fetches; `object-src 'none'`, `base-uri 'none'`). The CSP allows `style-src 'unsafe-inline'` and `img-`/`font-src data:` because the Cloudscape/Vite bundle requires them. Asserted by `TestLoopbackHostGuardSecurityHeaders` (`cmd/cryptamap/serve_test.go`).
- The dashboard SPA is served from a `go:embed`-ed directory (`go:embed all:webdist`, `cmd/cryptamap/web_embed.go:18-19`), so a *released* binary serves the UI with no external asset fetch. **Build caveat (security-relevant, not a defect):** the committed `webdist/` contents are a **placeholder `index.html` only** (`cmd/cryptamap/web_embed.go:5-13`); the real Vite bundle is staged into `cmd/cryptamap/webdist` by `make build-serve` (Makefile:23-29) *before* `go build`. A plain `make build-cli` / vanilla `go build` therefore embeds only the placeholder and `cryptamap serve` renders a stub shell, not the real dashboard. This does **not** weaken the loopback/no-network guarantees above — whatever is embedded is still served only over `127.0.0.1` with zero network/AWS calls — but a reviewer evaluating the served UI must use a `make build-serve`/release binary. **The placeholder now fails loudly, and the release build guards against it:** an earlier committed `index.html` was a *stale copy of a real build* that referenced gitignored hashed `/assets/` bundles, so a clean-checkout `go build` embedded a shell whose scripts 404'd and `serve` rendered a silent blank page. The committed placeholder now references **no** hashed assets and renders a visible "dashboard not embedded — run `make build-serve`" page instead (`cmd/cryptamap/webdist/index.html:1-38`), and `scripts/release-build.sh` runs an embedded-dashboard preflight that **fails the release** (`exit 1`) if the staged `index.html` references any `/assets/…` file missing from the staged tree (`scripts/release-build.sh:78-88`) — so a stale index that would serve a broken UI can no longer ship.
...
|---|---|---|
| 1 | No write/mutate IAM action on scanned resources; evidence-store writes limited to append-only `s3:PutObject`/`s3:Abort*` and `dynamodb:PutItem` (no delete/update/legal-hold/retention) | `cdk/templates/scanner-role-template.json:90-234` (custom `CryptaMapScannerReadActions` read-only allowlist), `cdk/lib/security-stack.ts:92-110,234-256`, `cdk/lib/scanner-stack.ts:91-111` (scoped evidence writes) |
...
| 6 | `DataStack` exposes no query API (no API Gateway / query Lambda / routes / presigned path) | `cdk/lib/data-stack.ts:12-24` |
| 8 | Evidence store KMS-CMK encrypted, no public access, retained; writers inherit the CMK (no SSE override) and a bucket policy denies non-`aws:kms` puts | `cdk/lib/data-stack.ts:35-132` (CMK/bucket/table), `:93-104` (deny non-KMS put); `internal/output/s3_writer.go:39-46`, `cmd/cryptamap/lambda_merge.go:259-266` (no SSE override) |
...
| 12 | Data home region is operator-controlled (`CDK_DEFAULT_REGION`); CDK warns (does not block) when home or fan-out regions fall outside India (`ap-south-1`/`ap-south-2`) | `cdk/bin/app.ts:96,99-120`, `cmd/cryptamap/lambda.go:97-103` |
